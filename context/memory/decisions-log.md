...
- Dampak: route upload kini menerjemahkan `PrismaClientKnownRequestError` dari trigger menjadi `AppError` 400 via `translateDatabaseTriggerError`. Pada DB tanpa trigger (test mock), status awal yang ditulis service sama dengan hasil trigger pada DB nyata.
...
- Referensi: #319, REVIEW.md H-2, `prisma/migrations-v2/20260920031000_harden_v2_database_rules/migration.sql`.
