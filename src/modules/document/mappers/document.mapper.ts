...
export function mapDocumentRecord(record: any) {
  return {
    ...
    fileName: record.storedFile?.fileName ?? record.fileName,
    fileSize: bigIntToNumber(record.storedFile?.fileSize ?? record.fileSize),
...
export function mapDocumentDetail(record: any) {
  return {
    ...
    issueDate: record.issueDate ? record.issueDate.toISOString?.() ?? record.issueDate : null,
    mimeType: record.storedFile?.mimeType ?? record.mimeType,
